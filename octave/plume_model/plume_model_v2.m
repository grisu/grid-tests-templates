...
%hours=48/3600          % number of hours to integrate over
hours_timesteps=hours*3600/time_step   
%
%
plot_hours=2/3600                        % 1 plot per 2 second
plot_hours=1                             % 1 plot per hour
...
w_e_i=zeros(1,length(theta));
thickness_i=zeros(1,length(theta));
...
%
density_background=sigma(0*pressure,theta,salt);
%density_background=sigma_thermobaric(0*pressure,theta,salt);
%
%
count=0
count_output=0
...
%
density_ip1=sigma(0*pressure,theta_i,salt_i);
%density_ip1=sigma_thermobaric(0*pressure,theta_i,salt_i);
%
...
% as a fixed fraction of the downslope speed
%w_e_ip1=-0.00001*u_ip1;
%w_e_ip1=-0.0001*u_ip1;
w_e_ip1=-0.001*u_ip1;
%
% Solve the continuity equation
% dH/dt=-dU/dx+w_e
%
U_transport_ip1=thickness_i.*u_ip1;
thickness_ip1=solve_continuity_equation(thickness_i,U_transport_ip1,w_e_ip1,distance, time_step);

diffusion=thickness_i;

source=zeros(size(thickness_i));
%source(1)=-2.0*(thickness_i(2)-thickness_i(1));
%source(n)=2.0*(thickness_i(n)-thickness_(n-1));

diffusion=K_h_thickness*(diffusion(jp1)-2*diffusion(j)+diffusion(jm1)+source);

thickness_ip1=thickness_ip1+diffusion;

%
%
theta_ip1=solve_heat_plume(theta_i,theta,u_ip1,w_e_ip1,distance,thickness_ip1,time_step);
...
%theta_ip1=-time_step*solve_heat_plume(theta_tmp,theta_m,u_ip1,w_e_ip1,distance,thickness_ip1);
...
%
%
salt_ip1=solve_heat_plume(salt_i,salt,u_ip1,w_e_ip1,distance,thickness_ip1,time_step);
%salt_tmp=salt_tmp/2+(salt_i(indexj)+salt_i(indexjp1))/2;
%salt_m=(salt_i(indexj)+salt_i(indexjp1))/2;
%salt_ip1=-time_step*solve_heat_plume(salt_tmp,salt_m,u_ip1,w_e_ip1,distance,thickness_ip1);
...
w_field(count_output,:)=w_e_ip1;
thickness_field(count_output,:)=thickness_ip1;
...
save results_plume_model theta_field salt_field delta_dens_field ...
     ...
     salt theta slope dthetadt_field thickness_field
